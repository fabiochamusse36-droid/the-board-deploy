...
foreach ($item in $frontendItems) {
  $destination = Join-Path 'frontend' $item
  if ((Test-Path $item) -and !(Test-Path $destination)) {
...
if (Test-Path 'apps/api') {
  $backendItems = @(Get-ChildItem 'backend' -Force -ErrorAction SilentlyContinue)
  if ((Test-Path 'backend') -and ($backendItems.Count -eq 0)) {
...
if (Test-Path 'backend/prisma') {
  if (Test-Path 'database/prisma') {
    Remove-Item 'database/prisma' -Recurse -Force
  }
...
Write-Host 'Next step: review git status before committing.' -ForegroundColor Yellow
