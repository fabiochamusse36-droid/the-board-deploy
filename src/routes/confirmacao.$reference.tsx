...
// Internal state machine (never surfaced to user copy).
// Simulates the transition period that will later be owned by Gateway RW.
type Stage =
  | "reservation_created"
  | "payment_preparing"
  | "payment_processing"
...
function paymentLabelPt(method: string): string {
  switch (method) {
    case "mpesa":
      return "M-Pesa";
    case "emola":
...
function Confirmacao() {
  ...
  // Sequential internal progression: created → preparing → processing → confirmed.
  useEffect(() => {
    if (!order) return;
    ...
    const timers: number[] = [];
    timers.push(
      window.setTimeout(() => setStage("payment_preparing"), 900),
    );
    timers.push(
      window.setTimeout(() => setStage("payment_processing"), 2100),
    );
    timers.push(
      window.setTimeout(() => {
  ...
  // Auto-redirect once confirmed; show fallback link if redirect stalls.
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground py-24 px-6">
      <div className="max-w-2xl mx-auto">
        ...
        {/* Reservation summary — pure status, no gateway content */}
        <div className="border border-gold/60 bg-gradient-to-b from-card to-background p-8 md:p-10 mb-8 text-center shadow-gold">
          ...
          <p className="font-display text-3xl md:text-4xl text-gradient-gold mt-3 tracking-widest break-all">
            {order.reference}
          </p>
          ...
          <dl className="grid grid-cols-2 gap-y-3 text-sm text-left max-w-sm mx-auto">
            ...
            <dd className="text-right font-display">
              {order.amount_mt.toLocaleString("pt-PT")} MT
            </dd>
            <dt className="text-muted-foreground">Canal preferido</dt>
            <dd className="text-right">{paymentLabelPt(method)}</dd>
          </dl>
        </div>

        {/* Fallback link — only appears AFTER payment is confirmed */}
        ...
        <div className="border border-border/40 bg-background/60 p-5 mb-10 text-xs text-muted-foreground leading-relaxed">
          A admissão final está sujeita à validação executiva. Caso o perfil não seja
          elegível para a categoria solicitada, o valor será reembolsado conforme a
          política do evento.
        </div>
  ...
  switch (stage) {
    case "reservation_created":
      return {
        ...
        body: "A sua reserva foi registada com sucesso.",
      };
    case "payment_preparing":
      return {
        eyebrow: "Preparando pagamento",
        title: "Preparando pagamento",
        body: "A preparar a continuação segura do pagamento.",
      };
    case "payment_processing":
      return {
        eyebrow: "Aguardando confirmação",
        title: "Aguardando confirmação",
        body: "Aguardamos a confirmação do pagamento associado a esta reserva.",
      };
...
function Journey({ stage }: { stage: Stage }) {
  ...
  const steps: { label: string; state: StepStatus; hint: string }[] = [
    { label: "Reserva", state: "done", hint: "concluída" },
    {
      ...
      hint: paymentDone ? "concluído" : "em curso",
    },
    {
      label: "Admissão",
      state: admissionActive ? "active" : "locked",
      hint: admissionActive ? "disponível" : "bloqueada",
    },
