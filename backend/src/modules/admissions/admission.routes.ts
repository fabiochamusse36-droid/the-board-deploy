...
const submitAdmissionSchema = z.object({
  ...
  investmentExperience: z.string().trim().max(1000).optional(),
  motivation: z.string().trim().max(1500).optional(),
});
...
function admissionCanBeSubmitted(reservation: {
  ...
  return (
    reservation.paymentStatus === "payment_confirmed" &&
    ["admission_form_available", "admission_unlocked", "admission_pending", "admission_form_locked"].includes(reservation.admissionStatus)
  );
}

export async function admissionRoutes(app: FastifyInstance) {
  app.get("/api/admissions/access/:reference", async (request, reply) => {
    const params = z.object({ reference: z.string().trim().min(4).max(40) }).parse(request.params);
    ...
    const existingAdmission = reservation.admissions[0] ?? null;

    return ok(reply, {
      reference: reservation.reference,
      paymentStatus: reservation.paymentStatus,
      admissionStatus: reservation.admissionStatus,
      admissionAvailable: reservation.paymentStatus === "payment_confirmed" && !existingAdmission,
      existingAdmission,
    });
  });
  ...
  app.post("/api/admissions", async (request, reply) => {
    ...
    const reservation = await prisma.reservation.findUnique({
      where: { reference: input.reservationReference },
      include: {
        admissions: {
          orderBy: { submittedAt: "desc" },
          take: 1,
    ...
    if (existingAdmission) {
      return fail(reply, "Esta reserva já tem uma admissão submetida", 409, "admission_already_submitted");
    }

    if (!admissionCanBeSubmitted(reservation)) {
      return fail(reply, "Admissão indisponível para o estado atual da reserva", 409, "admission_not_available");
    }
