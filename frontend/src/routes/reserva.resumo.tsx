...
import { saveReservation } from "@/lib/reservations.mock";

export const Route = createFileRoute("/reserva/resumo")({
  head: () => ({
    meta: [
      { title: "Confirmar Reserva — THE BOARD 2026" },
      { name: "description", content: "Reveja os dados finais e confirme a sua reserva." },
    ],
  }),
...
function ReservaResumo() {
  ...
  async function confirmReservation(e: React.FormEvent) {
    ...
    setError(null);
    if (!draft) return;
    if (!draft.buyerName || !draft.buyerEmail || !draft.buyerPhone || !draft.country || !draft.city) {
        ...
        <form onSubmit={confirmReservation} className="mt-10 space-y-6">
          ...
          <section className="border border-border/40 bg-card/40 p-6 md:p-8">
            <div className="flex items-center justify-between gap-4 mb-5">
              <p className="text-[10px] tracking-[0.4em] uppercase text-gold">Participante</p>
              <Link to="/reserva/dados" className="text-[10px] tracking-widest uppercase text-muted-foreground hover:text-gold">
                Editar dados
              </Link>
            </div>
            {hasParticipant ? (
              <>
                <ReviewRow label="Nome" value={draft.buyerName ?? ""} />
                <ReviewRow label="Email" value={draft.buyerEmail ?? ""} />
                <ReviewRow label="WhatsApp" value={draft.buyerPhone ?? ""} />
                <ReviewRow label="País" value={draft.country ?? ""} />
                <ReviewRow label="Cidade" value={draft.city ?? ""} />
              </>
            ) : (
          ...
          <section className="border border-border/40 bg-background/60 p-6 md:p-8">
            <p className="text-[10px] tracking-[0.4em] uppercase text-gold mb-4">Política</p>
            ...
            <label className="flex items-start gap-3 cursor-pointer select-none">
              <input type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} className="mt-1 accent-[color:var(--color-gold,#c9a449)]" />
              <span className="text-xs text-muted-foreground leading-relaxed">
                Confirmo os dados da reserva e estou ciente de que a admissão final está sujeita à validação da Direção Executiva.
              </span>
            </label>
          </section>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <Link to="/reserva/dados" className="py-4 border border-border/60 text-muted-foreground tracking-widest text-xs uppercase text-center hover:text-gold hover:border-gold/40 transition">
              Voltar
            </Link>
            <button
              type="submit"
